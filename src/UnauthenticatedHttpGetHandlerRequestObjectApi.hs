...
-- | Handler for the UnauthenticatedHttpGetHandlerRequestObject kbapi query.
-- GET counterpart of 'UnauthenticatedHttpPostHandlerRequestObjectApi'.
-- Same 3-line decoding shape ("GetHandler / Request / Url") -- no auth
-- metadata because there is no auth call to name. Reuses the shared
-- 'Content.FoundHttpGetHandlerRequestObjectMatch' record type.
module UnauthenticatedHttpGetHandlerRequestObjectApi
    ( query
    ) where
...
query :: Content.UnauthenticatedHttpGetHandlerRequestObject -> ApiEnv QueryResult
query (Content.UnauthenticatedHttpGetHandlerRequestObject _ limit) = do
    kbFilename <- asksKbFilename
    liftIO (putStrLn ("[queryengine][api] UnauthenticatedHttpGetHandlerRequestObject using kb: " ++ kbFilename))
    ...
    pure (FoundUnauthenticatedHttpGetHandlerRequestObject Content.FoundUnauthenticatedHttpGetHandlerRequestObject
        { Content.foundUnauthenticatedHttpGetHandlerRequestObjectTotal = fromIntegral (length matches)
        , Content.foundUnauthenticatedHttpGetHandlerRequestObjectMatches = matches
        })

instantiateTemplate :: FilePath -> Word -> IO T.Text
instantiateTemplate kbFilename limit = do
    template <- TIO.readFile "templates/templateUnauthenticatedHttpGetHandlerRequestObject.pl"
